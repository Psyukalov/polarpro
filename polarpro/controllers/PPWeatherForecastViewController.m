//
//  PPWeatherForecastViewController.m
//  polarpro
//
...
@interface PPWeatherForecastViewController () <iCarouselDataSource, iCarouselDelegate, PPCustomSettingsViewControllerDelegate, PPAlertViewDelegate, AppListener, UIScrollViewDelegate, VPRefreshControlDelegate>
...
@implementation PPWeatherForecastViewController
...
- (void)setup {
    ...
    [self.navigationItem setTitle:LOCALIZE(@"weather_forecast")];
...
- (UIView *)carousel:(iCarousel *)carousel
  viewForItemAtIndex:(NSInteger)index
    ...
    if (!view) {
        view = [[PPWeatherForecastItemView alloc] initWithFrame:carousel.bounds];
    }
    PPWeatherForecastItemView *weatherForecastItemView = (PPWeatherForecastItemView *)view;
    if (_locationModel.locations.count > 0 && index <= _locationModel.locations.count - 1) {
        [weatherForecastItemView setLocation:_locationModel.locations[index]];
    }
    return weatherForecastItemView;
}
...
- (void)settingsBBI_TUI {
    PPCustomSettingsViewController *settingVC = [[PPCustomSettingsViewController alloc] initWithType:PPSettingsTypeWeatherForecast];
